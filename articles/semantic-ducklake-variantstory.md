...
release-to-release comparisons. The recommended boundary is:
...
4.  export the source-rich disease-decision Parquet relation;
5.  register it into a DuckLake staging table without copying;
6.  merge changed keys into the persistent decision table in one
    publication transaction; and
7.  retain the policy/profile and source receipt in snapshot metadata.
...
DBI::dbExecute(con, "USE clinvar_lake")
```

The enhanced exporter is the package boundary:

``` r

enhanced_path <- tempfile(fileext = ".parquet")
receipt <- rclinvarbitration_export_clinvarbitration_parquet(
  con,
  enhanced_path,
...
It writes one disease-specific decision per assembly locus with a stable
`record_key`, a complete-row `content_sha256`, policy identity, decision
counts, and nested SCV, RCV, and gene receipts. SCVs are attached to the
exact disease decision. RCVs remain allele-level receipts and carry
their own disease keys; their presence does not assert that an SCV
condition and an RCV condition are equivalent. The ClinVar release
receipt is returned separately and belongs in the DuckLake snapshot
metadata.

The [RGenomicsETL `ducklake-r`
fork](https://github.com/RGenomicsETL/ducklake-r) provides atomic
Parquet registration. `add_data_files(..., create = TRUE)` creates the
staging schema and transfers the existing Parquet file to DuckLake
without copying it or collecting rows into R:
...
ducklake::with_transaction({
  ...
  DBI::dbExecute(con, "DELETE FROM clinvar_incoming")
}, author = "RClinVarbitration",
   commit_message = "Publish ncbi-vcv-2026-07-02 default policy",
   commit_extra_info = paste(
     "release_id=ncbi-vcv-2026-07-02",
     paste0("policy_version=", receipt$policy_version),
     sep = ";"
   ))
```

The first `add_data_files(..., create = TRUE)` is a one-time
staging-schema snapshot. For later releases, call
`add_data_files("clinvar_incoming", next_path)` at the start of the
publication transaction. The delete and merge leave byte-identical rows
untouched and clear staging after use.

Each committed publication is one DuckLake snapshot.
`get_table_changes("clinvar_decisions", snapshot_id, snapshot_id)`
returns its exact `insert`, `delete`, `update_preimage`, and
`update_postimage` rows. Inspect the nested SCV and RCV receipts to
explain a change. This is DuckLake’s native change feed, not a
package-local delta API.

Source and derived identities should not be conflated. A new arbitration
profile can produce a new snapshot without pretending the NCBI source
changed. A release comparison holds the profile fixed; a policy
experiment holds the source snapshot fixed. RClinVarbitration therefore
has no hard DuckLake dependency for local single-release analysis.
